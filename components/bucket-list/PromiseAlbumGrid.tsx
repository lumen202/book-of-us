...
/**
 * The grid on a kept promise's own album page. A trimmed sibling of
 * `MemoryGrid`/`MemoryCard` — same corner-mount/tilt look and the same
 * `PhotoLightbox` on tap, but no reactions, no comments, no press-and-hold,
 * no remove control. This page is view-and-add only for now; removing a
 * photo from an album isn't in scope yet (see
 * `docs/agent/codebase-map/bucket-list.md`) and can reuse the existing
 * `softDeleteMemory` mutation later with no new backend work.
 */
